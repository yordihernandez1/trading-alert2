...
def analizar_ticker(ticker):
    ...
    try:
        ...
        if df.empty:
            print(f"❌ {ticker}: dataframe vacío")
            return None
        if len(df) < 20:
            print(f"❌ {ticker}: menos de 20 datos disponibles")
            return None
    except Exception as e:
        print(f"❌ {ticker}: error al descargar datos - {e}")
        return None

    if any(col not in df.columns for col in ["High", "Low", "Close"]):
        print(f"❌ {ticker}: columnas necesarias no presentes")
        return None
    ...
    try:
        rsi = ta.momentum.RSIIndicator(close=close.squeeze()).rsi().squeeze()
        macd_obj = ta.trend.MACD(close=close.squeeze())
        macd = macd_obj.macd().squeeze()
        macd_signal = macd_obj.macd_signal().squeeze()
        sma_50 = ta.trend.SMAIndicator(close=close.squeeze(), window=50).sma_indicator().squeeze()
        sma_200 = ta.trend.SMAIndicator(close=close.squeeze(), window=200).sma_indicator().squeeze()
        atr = ta.volatility.AverageTrueRange(high=high.squeeze(), low=low.squeeze(), close=close.squeeze()).average_true_range().squeeze()
    except Exception as e:
        print(f"❌ {ticker}: error al calcular indicadores - {e}")
        return None
    ...
    try:
        ...
        print(f"❌ {ticker}: error al extraer valores finales - {e}")
        return None
